...
export default function BreadCrumbs() {
    ...
    return (
        <motion.div 
            className="flex items-center gap-2 my-2 text-lg overflow-x-auto no-scrollbar"
...
function BreadCrumbLink({to, children} : {to: string, children: ReactNode}) {
    return (
        <Link to={to} state={{backlink : true}} className='px-1 hover:bg-sky-300 dark:hover:bg-sky-800 active:bg-sky-500 dark:active:bg-sky-300 rounded-lg min-w-max'>
            {children}
        </Link>
    )
}
...
function BreadCrumbArrow() {
    return (
        <svg width="9" height="14" viewBox="0 0 9 14" fill="none" xmlns="http://www.w3.org/2000/svg" className='min-w-max'>
